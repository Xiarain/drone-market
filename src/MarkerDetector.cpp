...
MarkerDetector::MarkerDetector(void)
{
	...
	// m_camMat = (Mat_<float>(3,3) << 569.505981,0,332.243208,
	// 								0,571.420258,248.176028,
	// 								0,0,1);
	...
	m_markerCorners3d.push_back(cv::Point3f(-length,-length,0));
	m_markerCorners3d.push_back(cv::Point3f(length,-length,0));
	m_markerCorners3d.push_back(cv::Point3f(length,length,0));
	m_markerCorners3d.push_back(cv::Point3f(-length,length,0));
}
...
Vec6f MarkerDetector::getTransformations(void)
{

	float sy = sqrt(m_markers[0].m_rotation(0,0) * m_markers[0].m_rotation(0,0) 
				 +  m_markers[0].m_rotation(1,0) * m_markers[0].m_rotation(1,0) );
    ...
    if (!singular)
    {
        x = atan2(m_markers[0].m_rotation(2,1) , m_markers[0].m_rotation(2,2));
        y = atan2(-m_markers[0].m_rotation(2,0), sy);
        z = atan2(m_markers[0].m_rotation(1,0), m_markers[0].m_rotation(0,0));
    }
    else
    {
        x = atan2(-m_markers[0].m_rotation(1,2), m_markers[0].m_rotation(1,1));
        y = atan2(-m_markers[0].m_rotation(2,0), sy);
        z = 0;
    }
    Vec6f euler(m_markers[0].m_translation[0], m_markers[0].m_translation[1], m_markers[0].m_translation[2], x, y, z);

    return euler;
//	return m_markers[0].m_rotation; //by xiarui  m_translation
...
bool MarkerDetector::findMarkers(const Mat& _frame, vector<Marker>& _detectedMarkers)
{
    ...
    // step2:转为灰度图,自适应的阈值,这个自适应阈值函数好像有问题啊。。。。！！！！！！ opencv
...
// 	imshow("markerCandidate", markerCandidateImg);
    ...
    // step5:检测marker，根据marker的id信息筛选
...
// 		Mat_<float> Tvec;
		Vec3f Tvec;
		Mat raux, taux;
        // 建立一个3d to 2d的映射 opencv library
